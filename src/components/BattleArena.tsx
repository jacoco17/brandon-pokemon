...
const BattleArena: React.FC = () => {
  ...
  useEffect(() => {
    const fetchTeam = async () => {
      try {
        ...
        const teamData = response.data.slice(0, 6);
        const pokemonData = await Promise.all(
          teamData.map(async (pokemon: { pokemonId: number, id: number }) => {
            try {
              ...
              console.error(`Error fetching Pokemon ${pokemon.pokemonId}:`, error);
              return null;
            }
          })
        );
        // Remove duplicates by pokemonId
        const uniqueTeam = pokemonData.filter((p, idx, arr) =>
  ...
  const getRandomPokemon = async () => {
    try {
      ...
      toast({
        title: 'Error',
        description: 'Failed to fetch random Pokémon',
        status: 'error',
        duration: 3000,
        isClosable: true,
      });
