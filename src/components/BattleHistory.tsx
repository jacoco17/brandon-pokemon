  ...
  return (
    <Box maxW="1200px" mx="auto" px={4}>
      <Flex justify="space-between" align="center" mb={6}>
        <Heading color="black">Battle History</Heading>
        {battles.length > 0 && (
          <Button
            colorScheme="red"
            variant="outline"
            ...
            <AccordionItem key={battle.id} bg="whiteAlpha.200" mb={4} borderRadius="lg">
              ...
              <AccordionPanel>
                {battle.battleDetails ? (
                  <Table variant="simple" size="sm" bg="white" color="black" borderRadius="lg">
                    <Thead>
                      <Tr>
                        <Th color="gray.700">Stat</Th>
                        <Th color="gray.700" isNumeric>{battle.pokemon1}</Th>
                        <Th color="gray.700" isNumeric>{battle.pokemon2}</Th>
                        <Th color="gray.700">Winner</Th>
                      </Tr>
                    </Thead>
                    <Tbody>
                      {battle.battleDetails.map((detail, index) => (
                        <Tr key={index}>
                          <Td color="black" textTransform="capitalize">{detail.stat}</Td>
                          <Td color="black" isNumeric>{detail.pokemon1Value}</Td>
                          <Td color="black" isNumeric>{detail.pokemon2Value}</Td>
                          <Td>
                            <Badge
                  ...
                  <Text color="gray.600">No detailed stats available for this battle.</Text>
                )}
              </AccordionPanel>
            </AccordionItem>
          ))}
